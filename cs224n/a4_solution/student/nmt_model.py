...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(
            embed_size, self.hidden_size, bidirectional=True, bias=True
        )
    ...
    def decode(
        ...
        for t, Y_t in enumerate(torch.split(Y, 1)):
            Y_t = torch.squeeze(Y_t, 0)  # from (1, b, e) to (b, e)
    ...
    def step(
        ...
        e_t_unsqueezed = torch.bmm(enc_hiddens_proj, dec_hidden_unsqueezed)
        e_t = torch.squeeze(e_t_unsqueezed, dim=2) # (b, src_len)
        ...
        a_t = torch.squeeze(torch.bmm(alpha_t_unsqueezed, enc_hiddens), dim=1) # (b, 2h)
