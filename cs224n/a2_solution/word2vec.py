...
def naiveSoftmaxLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors, dataset):
    ...
    prob = softmax(np.matmul(outsideVectors, centerWordVec))
    prob_out = prob[outsideWordIdx]
    loss = -np.log(prob_out)
    one_center = np.zeros_like(prob)
    one_center[outsideWordIdx] = 1
    gradCenterVec = np.matmul(outsideVectors.T, prob - one_center)
    gradOutsideVecs = np.matmul(prob[:, None], centerWordVec[None])
...
def negSamplingLossAndGradient(
    ...
    for i in indices:
        dotProduct = np.dot(outsideVectors[i], centerWordVec)
        sDotProduct = sigmoid(dotProduct)
        nsDotProduct = sigmoid(-dotProduct)
        if i == outsideWordIdx:
            loss += -np.log(sDotProduct)
            gradCenterVec += outsideVectors[i] * (sDotProduct - 1)
            gradOutsideVecs[i] += (sDotProduct - 1) * centerWordVec
        else:
            loss += -np.log(nsDotProduct)
            gradCenterVec += outsideVectors[i] * (1 - nsDotProduct)
            gradOutsideVecs[i] += (1 - nsDotProduct) * centerWordVec
...
def skipgram(
    ...
    centerVector = centerWordVectors[center_ind]
    for w in outsideWords:
        w_ind = word2Ind[w]
        lossTemp, gradCenterVecsTemp, gradOutsideVectorsTemp = word2vecLossAndGradient(
            centerVector, w_ind, outsideVectors, dataset
        )
        loss += lossTemp
        gradCenterVecs[center_ind] += gradCenterVecsTemp
